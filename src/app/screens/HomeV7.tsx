    ...
    return (
        <header className="sticky top-0 z-[60] hidden lg:block">
            <div className="mx-auto max-w-[1600px] px-6 py-4 flex items-center justify-between">
                <button onClick={() => navigateTab('home')} className="flex items-center gap-3 active:scale-95 transition-transform">
                ...
                <nav className="flex items-center">
                    <div className="flex items-center gap-1 p-1 bg-white/[0.03] border border-white/5 rounded-full backdrop-blur-2xl shadow-2xl">
                        {navItems.map((item) => (
                            <button
                                key={item.id}
                                onClick={() => navigateTab(item.id as TabRoute)}
                                className={cn(
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => navigate('profile')} className="flex items-center gap-3 pl-1 pr-4 py-1.5 bg-white/[0.03] border border-white/5 rounded-full hover:bg-white/10 transition-all">
...
function MobileHeader() {
    const { navigateTab, navigate } = useNavigation();
    return (
        <header className="sticky top-0 z-[60] lg:hidden px-4 py-3 pb-[calc(12px+env(safe-area-inset-top))]">
            <div className="flex items-center justify-between mt-[env(safe-area-inset-top)]">
                <button onClick={() => navigateTab('home')} className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <button onClick={() => navigate('profile')} className="w-9 h-9 rounded-full overflow-hidden border border-white/10 active:scale-90">
            ...
            <div className="flex items-center justify-around h-16 px-1">
                {navItems.map((item) => {
                    ...
                    return (
                        <button
                            key={item.id}
                            onClick={() => item.id === 'profile' ? navigate('profile') : navigateTab(item.id as TabRoute)}
